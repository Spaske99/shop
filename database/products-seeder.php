...
try {
    ...
    foreach ($products as $product) {
        $sqlQueryString = "INSERT INTO products_tb (title, description, img, price, category, type,  stars, status) VALUES (?,?,?,?,?,?,?,?);";
        $statement = $connection->prepare($sqlQueryString);
        $status = $statement->execute([$product["title"], $product["description"], $product["img"], $product["price"], $product["category"], $product["type"], $product["stars"], $product["available"]]);
    }
